...
abstract class Timer extends Handler {
	...
	/**
	 * Start the timer. step() will be called at regular intervals until it
	 * returns true; then done() will be called.
	 * <p/>
	 * Subclasses may override this to do their own setup; but they must then
	 * call super.start().
	 */
	public void start() {
		if (mIsRunning)
	...
	/**
	 * Stop the timer. step() will not be called again until it is restarted.
	 * <p/>
	 * Subclasses may override this to do their own setup; but they must then
	 * call super.stop().
	 */
	 ...
	 * 
	 * @param count
	 *            The call count; 0 on the first call.
	 * @param time
	 *            The total time for which this timer has been running, in ms.
	 *            Reset by reset().
	 * @return true if the timer should stop; this will trigger a call to
	 *         done(). false otherwise; we will continue calling step().
	 */
	...
	/**
	 * Restore our game state from the given Bundle. If the saved state was
	 * running, we will continue running.
	 * 
	 * @param map
	 ...
	 *            A Bundle containing the saved state.
	 * @param run
	 *            If true, restore the saved runnning state; otherwise restore
	 *            to a stopped state.
	 * @return true if the state was restored OK; false if the saved state was
	 *         incompatible with the current configuration.
	 */
